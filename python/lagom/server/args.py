...
import torch
from lagom.distributed import DistributedInfo
from lagom.scheduler import SchedulerConfig
...
class ServerArgs(SchedulerConfig):
    ...
    def zmq_frontend_addr(self) -> str:
        return "ipc:///tmp/lagom_3" + self._unique_suffix

    @property
    def zmq_tokenizer_addr(self) -> str:
        if self.share_tokenizer:
            return self.zmq_detokenizer_addr
        result = "ipc:///tmp/lagom_4" + self._unique_suffix
...
def parse_args(args: List[str], run_shell: bool = False) -> Tuple[ServerArgs, bool]:
    ...
    """
    from lagom.attention import validate_attn_backend
    from lagom.kvcache import SUPPORTED_CACHE_MANAGER
    from lagom.moe import SUPPORTED_MOE_BACKENDS

    parser = argparse.ArgumentParser(description="Lagom Server Arguments")
    ...
    if (dtype_str := kwargs["dtype"]) == "auto":
        from lagom.utils import cached_load_hf_config
